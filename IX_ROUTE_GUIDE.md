...
脚本应在出口 VPS 上执行。适用于 Mieru、Trojan、AnyTLS 或其他以中国侧公网入口映射到出口 VPS 的 TCP 服务。Mieru／Mita 检查是可选附加项；未安装 Mieru 不影响入口和同流返回检测。
...
脚本使用该探针收到的 TCP 终点回应或追加三次 TCP 业务端口测量，确认回应已返回同一探针。

这能证明同一 TCP 流量可双向建立，但第三方测量 API 无法显示反向逐跳路由。因此报告会明确写出“反向逐跳不可见”，不会虚构路径。
...
| 状态 | 含义 |
|---|---|
| `PASS` | 原省指定运营商探针到达入口，或同流 TCP 返回已确认 |
...
- TCP traceroute 的终点回应本身包含返回过程，但不提供反向逐跳拓扑。
- 中间路由器不回应不等于业务丢包。
