...
CMD、本地 HTML 与公共报告不再用大段证据文字堆叠主要数据：

- 主标题固定显示 `中国三网探针 → 脱敏入口IP:脱敏端口`。
- 六地区 × 三网 RTT 合并为 6×3 色彩网格；每格保留实际探针省市、ASN、脱敏入口及精确／跨省备援状态。
- TCP 应答并入同一证据格，只显示“已收到／未确认”与“反向逐跳不可见”，不重复 RTT。
- 单线程速度与重传使用网格渐层条；`N/A` 保持灰色，不用 0 填补缺测。
- 顶部将“三网运营商探针可达”与“全部测点 TCP 应答”分开；后者明确注明包含多少第三方参考点。
- 运营商卡将“运营商探针覆盖／应答”与“第三方参考点应答”分开；第三方机房不再计入中国电信、联通或移动探针应答。
- 单线程测速成功取得数据时标为 `VALID／已测得`，只表示数据有效，不代表速度、重传或线路质量合格。
- JSON 使用 `sameProvinceReachable` 表示同省有效测点，删除语义不清的 `reachable`。
- 改善建议改为短卡片，长证据与方法说明默认收起。

公开报告仍保存结构化证据；色彩只负责帮助比较，不改变 `PASS`、`VALID`、`PARTIAL`、`N/A` 等判定。
...
| `VALID` | 单线程辅助测速已取得可解析数据；不是质量合格判定 |
...
三网公网单线程速度（辅助项）
地区      回程重传       回程速度              去程速度
          VPS→中国测速端  VPS→中国测速端         中国测速端→VPS
北京电信       ...      ...Mbps     ...Mbps
```

报告会紧邻表格注明“不经过中国侧业务入口”。成功取得数据标为 `VALID／已测得`，失败项显示 `N/A`，不会以 `0 Mbps` 计入判定。

最终页分开显示：

- 三网运营商探针可达与原省／跨省探针覆盖。
- 全部有效测点 TCP 应答确认，并单列其中第三方参考点数量；明确注明非反向逐跳路由。
...
- 三网运营商探针12组与第三方参考点4组的应答统计分离
- `sameProvinceReachable` 取代语义不清的 `reachable`
- 公共报告三网映射
- 北上广三网九组单线程速度结构、方向箭头、`VALID` 数据状态及测量边界
- 最终评分／星级移除
