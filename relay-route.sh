...
用途：
  独立检测“中国用户 → 公网中转入口:入口端口 → WireGuard／隧道内段
  → 出口内网IP:后端端口 → 公网出口”。
  本脚本为独立的公网入口／隧道／出口后端检测工具。
  入口端口与后端端口分别核对，原生支持 10443 → 24443 等端口转换。

推荐：在出口 VPS 上执行完整六地区 × 三网测试
  bash <(curl -fsSL https://raw.githubusercontent.com/souldance7-ai/vps-speedtest/main/relay-route.sh) --full

非交互示例：
  bash relay-route.sh --entry 203.0.113.10 --entry-port 10443 \
    --local-private 10.0.0.2 --peer 10.0.0.1 --backend-port 24443 \
    --expected-exit 198.51.100.20 --client-verified-exit 198.51.100.20 --full

默认北上广三网TCP去程＋应答确认；完整六地区三网：
  bash relay-route.sh --entry 公网入口IPv4 --entry-port 入口业务端口 \
    --backend-port 出口后端端口 --full

可选参数：
  --entry IP             公网中转入口 IPv4
  --entry-port PORT      用户连接的入口业务端口
  --port PORT            --entry-port 的兼容别名
  --backend-port PORT    出口内网实际后端监听端口，可与入口端口不同
  --expected-exit IP     预期公网出口
  --local-private IP     出口端隧道内网 IP
  --peer IP              入口端隧道内网对端
...
判定边界：
  入口 TCP traceroute 与TCP应答只证明公网入口端口可达。
  出口端仅检查实际后端端口，不要求与公网入口端口相同。
  公网入口可达、出口内网地址吻合、后端监听、私网对端可达、客户端实测出口一致，
  才把完整中转链判为 PASS。
  不绑定特定守护进程，也不限定 Trojan、AnyTLS、REALITY 或其他 TCP 协议。
  ...
  公网中转入口，不代表实际中转端到端吞吐，不参与闭环 PASS。
  本地报告会把用户相关 IPv4 脱敏为前两段，业务端口末三位脱敏为 ***。
  报告保存到“Relay 3Net”目录，并生成 HTML／JSON／Markdown。
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --entry-port|--port) ENTRY_PORT="${2:-}"; shift 2 ;;
...
export RELAY_VERSION="$VERSION"
export RELAY_ENTRY_IP="$ENTRY_IP"
export RELAY_ENTRY_PORT="$ENTRY_PORT"
export RELAY_BACKEND_PORT="$BACKEND_PORT"
export RELAY_EXPECTED_EXIT="$EXPECTED_EXIT"
export RELAY_LOCAL_PRIVATE="$LOCAL_PRIVATE"
export RELAY_REMOTE_PEER="$REMOTE_PEER"
export RELAY_CLIENT_VERIFIED_EXIT="$CLIENT_VERIFIED_EXIT"
export RELAY_FULL="$FULL"
export RELAY_SELF_TEST="$SELF_TEST"
export RELAY_SPEED_TEST="$SPEED_TEST"
...
VERSION = os.environ.get("RELAY_VERSION", "v1.0.0")
ENTRY_IP = os.environ.get("RELAY_ENTRY_IP", "").strip()
PORT_TEXT = os.environ.get("RELAY_ENTRY_PORT", "").strip()
BACKEND_PORT_TEXT = os.environ.get("RELAY_BACKEND_PORT", "").strip()
EXPECTED_EXIT = os.environ.get("RELAY_EXPECTED_EXIT", "").strip()
LOCAL_PRIVATE = os.environ.get("RELAY_LOCAL_PRIVATE", "").strip()
REMOTE_PEER = os.environ.get("RELAY_REMOTE_PEER", "").strip()
CLIENT_VERIFIED_EXIT = os.environ.get("RELAY_CLIENT_VERIFIED_EXIT", "").strip()
FULL = os.environ.get("RELAY_FULL") == "1"
SELF_TEST = os.environ.get("RELAY_SELF_TEST") == "1"
SPEED_TEST = os.environ.get("RELAY_SPEED_TEST") == "1"
...
def ask_inputs() -> tuple[str, int, int, str, str, str]:
    global ENTRY_IP, PORT_TEXT, BACKEND_PORT_TEXT
    global EXPECTED_EXIT, LOCAL_PRIVATE, REMOTE_PEER
    if SELF_TEST:
        return (
            "203.0.113.10", 10443, 24443,
            "198.51.100.20", "10.77.77.1", "10.77.77.2",
        )

    section("INPUT GUIDE / 中转专用目标输入", MAGENTA)
    field(
        "检测架构",
        "中国用户 → 公网入口:入口端口 → 隧道内段 → 出口内网:后端端口 → 公网出口",
    )
    field("公网入口", "填写用户实际连接的中转入口 IPv4", CYAN)
    field("入口端口", "填写用户实际连接端口；不是 SSH 22 或管理端口", GREEN)
    field("后端端口", "填写出口内网真实监听端口；允许与入口端口不同", GREEN)
    field("出口端内网", "填写出口 VPS 的 WireGuard／隧道内网 IPv4", CYAN)
    field("入口内网对端", "填写入口 VPS 的 WireGuard／隧道内网 IPv4", CYAN)
    field("预期出口", "填写出口 VPS 公网 IPv4；未知可留空", GREEN)

    while not valid_ipv4(ENTRY_IP, public=True):
        ENTRY_IP = input("请输入公网中转入口 IPv4：").strip()
        if not valid_ipv4(ENTRY_IP, public=True):
            print(RED + "  IPv4 无效，请重新输入。" + RESET)
    while True:
        if not PORT_TEXT:
            PORT_TEXT = input("请输入入口业务端口〔用户实际连接端口〕：").strip()
        try:
            port = int(PORT_TEXT)
            if 1 <= port <= 65535:
                break
        ...
        PORT_TEXT = ""
    while True:
        if not BACKEND_PORT_TEXT:
            BACKEND_PORT_TEXT = input(
                "请输入出口内网后端端口〔可与入口端口不同〕："
            ).strip()
        try:
            ...
            pass
        print(RED + "  后端端口无效，请输入 1～65535。" + RESET)
        BACKEND_PORT_TEXT = ""
    ...
    if REMOTE_PEER and not valid_ipv4(REMOTE_PEER):
        ...
        REMOTE_PEER = ""
    return (
        ENTRY_IP, port, backend_port,
        EXPECTED_EXIT, LOCAL_PRIVATE, REMOTE_PEER,
    )


def http_json(url: str, method: str = "GET", payload: Any = None, timeout: int = 25) -> Any:
    body = None
    headers = {"Accept": "application/json", "User-Agent": "relay-route/1.0"}
...
def backend_service(port: int) -> dict[str, Any]:
    """Inspect the actual backend listener without assuming any proxy software."""
    if SELF_TEST:
        return {
            "status": "PASS",
            "port": port,
            "transport": "TCP",
            "process": "xray",
            "evidence": f"SELF-TEST TCP {port} LISTEN｜process xray",
        }
    listener = listener_status(port)
    evidence = listener.get("evidence") or f"未发现后端端口 {port} 监听"
    process_match = re.search(
        r'users:\(\("([^"]+)"|process:\(\("([^"]+)"',
        evidence,
    )
    process = next(
        (value for value in (process_match.groups() if process_match else ()) if value),
        "N/A",
    )
    return {
...
def handshake_verdict(
    ...
    if not client_exit:
        return {
            "status": "N/A",
            "reason": "尚未提供中国客户端连接本中转节点后实测到的公网出口",
        }
    ...
    if backend.get("status") != "PASS":
        return {
            "status": "INCONCLUSIVE",
            "reason": "客户端出口一致，但出口内网后端端口监听尚未确认",
        }
    ...
    return {
        "status": "PASS",
        "reason": "客户端经公网入口与隧道后端连接，实测出口与出口 VPS 实际／预期一致",
    }
...
def single_thread_speed() -> dict[str, Any]:
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={"User-Agent": f"relay-route/{VERSION}"},
        )
        ...
        status = "VALID" if valid == 9 else "PARTIAL" if valid else "N/A"
        reason = (
...
def show_single_thread_speed(speed: dict[str, Any]) -> None:
    ...
    field(
        ...
        CYAN if speed.get("status") == "VALID" else YELLOW,
    )
    rows = speed.get("rows") or []
    if not rows:
        return
    ...
    for row in rows:
        ...
        if region != current_region:
            ...
            print(
                GRAY
                + f"  {'地区':<14}{'回程重传':>10}{'回程 VPS→中国测速端':>26}{'去程 中国测速端→VPS':>26}"
                + RESET
            )
...
def tcp_response_confirmation_from_probe(
    ...
    return {
        ...
        "evidenceRole": (
            "CARRIER_PROBE"
            if probe.get("status") in {"PASS", "PASS_FALLBACK"}
            else "THIRD_PARTY_REFERENCE"
            if probe.get("status") == "REFERENCE"
            else "UNAVAILABLE"
        ),
        "carrierRepresentative": bool(probe.get("carrierRepresentative")),
        "carrierReachabilityVerified": bool(
            probe.get("carrierReachabilityVerified")
        ),
...
def self_test_probe(carrier: str, region: str, city: str, index: int) -> dict[str, Any]:
    name, asn, _ = CARRIERS[carrier]
    no_probe = carrier == "CT" and index in {7, 16}
    reference = carrier == "CT" and not no_probe
    fallback = carrier != "CT" and index in {12, 14, 15, 17, 18}
    reached = not no_probe
    status = (
        "NO_PROBE" if no_probe
        else "REFERENCE" if reference
        else "PASS_FALLBACK" if fallback
        else "PASS"
    )
    source_class = (
        "NONE" if no_probe
        else "GENERIC_DATACENTER_REFERENCE" if reference
        else "CROSS_PROVINCE_CARRIER_FALLBACK" if fallback
        else "CARRIER_EYEBALL"
    )
    probe_region = "江苏" if fallback else region
    probe_city = "徐州" if fallback else city
    carrier_probe = status in {"PASS", "PASS_FALLBACK"}
    return {
        "carrier": carrier,
        "carrierName": name,
        "requestedRegion": region,
        ...
        "probeCity": "" if no_probe else probe_city,
        "probeState": "" if no_probe else probe_region,
        "probeRegion": "" if no_probe else probe_region,
        "probeAsn": 0 if no_probe else 45090 if reference else asn,
        "probeNetwork": "" if no_probe else "第三方省级参考点" if reference else name,
        "capitalPreferred": not (fallback or reference or no_probe),
        "selectorCity": city,
        "selectionRegion": region,
        "selectionTier": (
            "NONE" if no_probe
            else "PROVINCE_DATACENTER_REFERENCE" if reference
            else "CROSS_PROVINCE_CARRIER_EYEBALL" if fallback
            else "CAPITAL_CARRIER_EYEBALL"
        ),
        "sourceClass": source_class,
        "carrierRepresentative": status == "PASS",
        "carrierReachabilityVerified": carrier_probe,
        "regionFidelity": (
            "REGION_EXACT" if status == "PASS"
            else "CROSS_PROVINCE_FALLBACK" if fallback
            else "PROVINCE_REFERENCE" if reference
            else "UNVERIFIED"
        ),
        "carrierIdentityVerified": carrier_probe,
        "asnVerified": carrier_probe,
        "cityVerified": reached,
        "provinceVerified": reached and not fallback,
        "selectionProvinceVerified": reached and not fallback,
        ...
        "latency": {
            "status": "PASS" if reached else "N/A",
            "avg": round(18.0 + index * 1.7, 1) if reached else None,
            "p95": round(20.0 + index * 1.7, 1) if reached else None,
            "jitter": 1.2 if reached else None,
            "loss": None, "received": 3 if reached else 0, "sent": 3,
        },
        "route": (
        ...
        "reason": (
            "SELF-TEST 第三方省级参考点，不冒充中国电信探针"
            if reference
            else "SELF-TEST 跨省同运营商备援，非原省代表"
            if fallback
            else "SELF-TEST 无可用探针"
            if no_probe
            else ""
        ),
    }
...
def mapping_chain(
    ...
    reachable = access.get(
        "endpointReachable",
        access.get("sameProvinceReachable", access.get("pass", 0)),
    )
    if (
        reachable > 0
        ...
        return {
            "status": "PARTIAL",
            "reason": (
                f"公网入口端口 {mask_port(entry_port)} 可达＋出口后端端口 "
                f"{mask_port(backend_port)} 监听＋出口私网地址吻合＋隧道对端可达；"
                "尚缺客户端实测出口闭环"
            ),
        }
    ...
    if reachable > 0 and listener.get("status") == "PASS":
        return {
            "status": "PARTIAL",
            "reason": "公网入口端口可达且出口后端端口监听，但出口内网地址未核对",
        }
    if reachable > 0:
        return {
            "status": "INCONCLUSIVE",
            "reason": (
                f"公网入口端口 {mask_port(entry_port)} 可达，但出口后端端口 "
                f"{mask_port(backend_port)} 未确认监听；两端端口允许不同"
            ),
        }
    return {
        "status": "N/A",
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- 公网入口：`{report['entry']['masked']}:{report['entry']['port']}`",
        f"- 出口后端：`{report['backend']['privateMasked']}:{report['backend']['port']}`",
        ...
        "",
        (
            f"- 公网中转入口 TCP：{access['status']}（运营商可达 "
            f"{access.get('carrierReachable', access['pass'])}/{access['total']}，"
            f"原省精确 {access['pass']}/{access['total']}，省级可达 "
            f"{access.get('sameProvinceReachable', access['pass'])}/{access['total']}，"
        ...
        (
            f"- 全部有效测点 TCP 应答确认：{response_summary['status']} "
            f"{response_summary['confirmed']}/{response_summary['total']}；"
            f"其中运营商探针 {response_summary.get('carrierConfirmed', 0)}，"
            f"第三方参考点 {response_summary.get('referenceConfirmed', 0)}；"
            "仅确认入口端口应答，非独立反向逐跳路由，不计算回程分数"
        ),
        (
            f"- 三网公网单线程速度：{report['singleThreadSpeed']['status']} — "
            f"{report['singleThreadSpeed']['reason']}；不经过公网中转入口，"
            "不参与中转闭环判定"
        ),
        f"- 出口后端监听：{listener['status']} — {listener['evidence']}",
        f"- 通用后端服务：{report['backendService']['status']} — {report['backendService']['evidence']}",
        f"- 客户端出口闭环：{report['protocolHandshake']['status']} — {report['protocolHandshake']['reason']}",
        f"- 完整中转链：{report['mappingChain']['status']} — {report['mappingChain']['reason']}",
        f"- 隧道纯内段：{internal['status']} — {quality_label(internal)}",
        ...
        *[f"- {item}" for item in report.get("improvements") or []],
        ...
        "## 中转链判定边界",
        "",
        "1. 中国探针到公网入口端口的 TCP traceroute 到达，只表示入口 TCP 路径可达。",
        "2. 公网入口端口与出口内网后端端口分开核对，允许两端使用不同端口。",
        "3. 提供入口隧道内网对端后，统计纯内段 Ping RTT／P95／抖动／丢包。",
        "4. 不限定代理软件；只有客户端连接中转节点后实测出口一致，出口闭环才为 PASS。",
        "5. traceroute 中间跳点不回应、DNS 或探针失败不记作 100% 业务丢包。",
        "6. 本脚本使用独立的中转链判定与报告结构。",
        "",
        "## 公网中转入口探针",
        "",
    ...
    lines.extend([
        ...
        "",
        "| 区域 | 测速节点 | 回程重传（VPS→中国测速端） | 回程速度（VPS→中国测速端） | 去程速度（中国测速端→VPS） | 数据状态 |",
        "|---|---|---:|---:|---:|---|",
    ])
    ...
    lines.extend([
        ...
        f"- 到公网中转入口路由：`{report['networkContext']['entryRoute']}`",
...
<title>Relay 3Net｜中转专用证据网格</title>
<style>
...
.PASS,.PASS_FALLBACK{color:var(--green)}.VALID{color:var(--cyan)}.FAIL{color:var(--red)}.N\\/A,.NO_PROBE,.INCONCLUSIVE,.PARTIAL,.REFERENCE{color:var(--yellow)}.panel{padding:18px;margin:14px 0}h2{font-size:15px;color:#ff63d8;margin:0 0 12px;letter-spacing:.08em}
...
<section class="panel"><h2>SPEED GRID / 三网公网单线程（辅助项）</h2><div class="scroll"><table class="speed"><thead><tr><th>区域</th><th>节点</th><th>回程重传<br>VPS→中国测速端</th><th>回程速度<br>VPS→中国测速端</th><th>去程速度<br>中国测速端→VPS</th><th>数据状态</th></tr></thead><tbody id="speedRows"></tbody></table></div><p class="note" id="speedBoundary"></p></section>
...
document.getElementById('entryNode').innerHTML=`公网中转入口<br><code>${E(endpoint)}</code>`;
document.getElementById('meta').textContent=`${R.generated} · ${R.version} · ${R.matrix}`;
document.getElementById('cards').innerHTML=[
 badge('三网运营商探针可达',R.access.status,`${R.access.carrierReachable??R.access.pass}/${R.access.total}`),
 badge('全部测点TCP应答',R.tcpResponseSummary.status,`${R.tcpResponseSummary.confirmed}/${R.tcpResponseSummary.total}（含参考点${R.tcpResponseSummary.referenceConfirmed??0}）`),
 badge('原省精确／跨省备援',R.access.regionStatus,`${R.access.pass}/${R.access.total} · ${R.access.crossProvinceFallback??0}`),
 ...
 badge('私网内段回程',R.internal.status,R.internal.status)
].join('');
...
const B=R.backendService;
document.getElementById('details').innerHTML=`<tr><th>公网入口</th><td><code>${E(endpoint)}</code></td><th>出口 VPS</th><td><code>${E(R.exitIdentity.ipMasked)}</code></td></tr><tr><th>出口后端</th><td class="${E(B.status)}">${E(B.status)} · ${E(R.backend.privateMasked)}:${E(R.backend.port)}</td><th>后端监听</th><td class="${E(R.listener.status)}">${E(R.listener.status)}</td></tr><tr><th>完整中转链</th><td class="${E(R.mappingChain.status)}">${E(R.mappingChain.status)}</td><th>客户端出口闭环</th><td class="${E(R.protocolHandshake.status)}">${E(R.protocolHandshake.status)}</td></tr>`;
document.getElementById('method').textContent=R.methodology;
document.getElementById('json').onclick=()=>{const a=document.createElement('a');a.download='relay-route-report.json';a.href=URL.createObjectURL(new Blob([JSON.stringify(R,null,2)],{type:'application/json'}));a.click()};
</script></body></html>"""
    path.write_text(
...
def main() -> int:
    ...
    print(BLUE + "\n  ═══════════════════════════════════════════════════════════════" + RESET)
    print(CYAN + f"  RELAY-ROUTE {VERSION} · 中转专用" + RESET)
    print(BLUE + "  ═══════════════════════════════════════════════════════════════" + RESET)

    section("TOPOLOGY / 检测拓扑", CYAN)
    field("公网中转入口", f"{mask_ip(entry)}:{mask_port(port)}", CYAN)
    field("入口业务端口", f"{mask_port(port)}（用户实际连接端口）", GREEN)
    field("出口后端端口", f"{mask_port(backend_port)}（允许与入口端口不同）", GREEN)
    field("预期公网出口", mask_ip(expected_exit), GREEN)
    field("出口端内网", mask_ip(local_private), CYAN)
    field("入口内网对端", mask_ip(remote_peer), MAGENTA)
    field(
        "测试边界",
        "TCP去程、原探针TCP应答、入口/后端分端口、隧道内段与客户端出口闭环；"
        "TCP应答不是反向逐跳路由",
        YELLOW,
    )
    ...
    addresses = ["10.77.77.1", "127.0.0.1"] if SELF_TEST else local_addresses()
    peer_source = "USER_PROVIDED" if remote_peer else ""
    if not remote_peer:
        remote_peer, peer_source = detect_private_peer(backend_port, addresses)
    local_status = (
    ...
    listener = (
        {"status": "PASS", "evidence": f"SELF-TEST TCP {backend_port} LISTEN"}
        if SELF_TEST else listener_status(backend_port)
    )
    backend = backend_service(backend_port)
    net_context = (
    ...
    section("LAYER 1 / 中国三网到公网中转入口", CYAN)
    ...
    carrier_response_pass = sum(
        item["status"] == "PASS" and item.get("evidenceRole") == "CARRIER_PROBE"
        for item in tcp_responses
    )
    reference_response_pass = sum(
        item["status"] == "PASS"
        and item.get("evidenceRole") == "THIRD_PARTY_REFERENCE"
        for item in tcp_responses
    )
    response_summary = {
        "status": (
        ...
        "carrierConfirmed": carrier_response_pass,
        "referenceConfirmed": reference_response_pass,
        "total": len(tcp_responses),
        "reason": (
            f"全部有效测点收到入口TCP应答 {response_pass}/{len(tcp_responses)}"
            f"（运营商探针 {carrier_response_pass}，第三方参考点 "
            f"{reference_response_pass}）；"
            "非反向逐跳路由，不计算回程分数"
        ),
        "reverseRouteVisible": False,
    }
    field(
        "全部测点TCP应答",
        f"{response_summary['status']} {response_pass}/{len(tcp_responses)}"
        f"（运营商探针 {carrier_response_pass}＋参考点 {reference_response_pass}）｜"
        "反向逐跳路由不可见，不评回程分",
        GREEN if response_summary["status"] == "PASS" else YELLOW,
    )
    ...
    access["sameProvinceReachable"] = (
        access["pass"] + access["provinceReference"]
    )
    ...
    access["sameProvinceCoverage"] = (
        round(access["sameProvinceReachable"] * 100 / total, 1)
        if total else 0.0
    )
    access["status"] = (
    ...
    section("LAYER 2 / 公网入口→隧道→出口后端", MAGENTA)
    field("出口内网本机核对", local_status, GREEN if local_status == "PASS" else YELLOW)
    field(
        "出口后端端口监听",
        f"{mask_port(backend_port)}｜{listener['status']}",
    ...
    internal = (
        {
            "status": "PASS",
            "reason": "SELF-TEST 隧道私网对端可达",
            "stats": summarize([22.0] * 20, 20),
            "route": "",
            "routeLookup": "10.77.77.2 dev wg-hktw src 10.77.77.1",
            "pathModel": "PRIVATE_PEER_SAME_PATH",
        }
    ...
    field(
        "中转链证据",
        f"{chain['status']}｜{chain['reason']}",
        GREEN if chain["status"] == "PASS" else YELLOW,
    )

    section("LAYER 3 / 通用后端与客户端出口闭环", MAGENTA)
    field(
        "后端服务监听",
        privacy_scrub(
            f"{backend['status']}｜{backend['evidence']}",
            runtime_sensitive_ips,
            [port, backend_port],
        ),
        GREEN if backend["status"] == "PASS" else YELLOW,
    )
    field(
        "TCP 业务监听",
        privacy_scrub(
    ...
    if handshake["status"] == "PASS" and chain["status"] == "PARTIAL":
        chain = {
            "status": "PASS",
            "reason": (
                "公网入口可达、出口后端监听、隧道内网核对及客户端实测出口"
                "已形成中转端到端闭环证据"
            ),
        }
    field(
        "客户端出口闭环",
        f"{handshake['status']}｜{handshake['reason']}",
        GREEN if handshake["status"] == "PASS" else YELLOW,
    )
    ...
    report = {
        ...
        "mode": "CN3_RELAY_ENTRY_BACKEND_EXIT_AUDIT",
        "matrix": (
        ...
        "backend": {
            "privateMasked": mask_ip(local_private),
            "port": backend_port,
            "service": backend,
        },
        ...
        "privacy": (
            ...
            "入口与后端业务端口末三位统一显示为***；报告不保存原值。"
        ),
        "methodology": (
            "中国侧使用 Globalping 指定中国电信、联通、移动完整运营商 ASN 家族，"
            "对公网中转入口业务端口执行 TCP traceroute；北京、上海、广州固定列入，"
            "--full 再扩展合肥、南京与杭州。每组TCP应答确认复用同一Globalping探针、"
            "同一入口与同一业务端口，只记录原探针是否收到入口TCP应答；"
            "这不是独立反向traceroute，不提供反向逐跳路由，也不计算回程分数。"
            "脚本不使用出口VPS默认公网路由探测其他中国公共目标来冒充本链路回程。"
            "出口 VPS 本机核对公网 IP、出口内网地址、后端真实监听端口、"
            "默认路由及到公网入口的路由。入口与后端端口分开核对，允许端口转换，"
            "不绑定特定守护进程，也不限定具体 TCP 代理协议。"
            "只有客户端连接本中转节点后实测出口与出口端实际／预期出口一致，"
            "才把客户端出口闭环判为 PASS。提供入口隧道内网对端，或后端端口存在可识别的"
    ...
    output_name = f"relay-route-report-{generated.strftime('%Y%m%d-%H%M%S')}"
    preferred_root = Path("/root") if os.geteuid() == 0 else Path.cwd()
    chain_root = preferred_root / "Relay 3Net"
    output_dir = chain_root / output_name
    try:
        output_dir.mkdir(parents=True, exist_ok=True)
    except OSError:
        output_dir = Path.cwd() / "Relay 3Net" / output_name
        output_dir.mkdir(parents=True, exist_ok=True)
    ...
    section("FINAL / 中转专用四层结论", CYAN)
    field(
        "公网中转入口接入",
        (
            f"{access['status']}｜三网运营商探针可达 {access['carrierReachable']}/{access['total']} "
    ...
    field("完整中转链", f"{chain['status']}｜{chain['reason']}", GREEN if chain["status"] == "PASS" else YELLOW)
    field("同路径私网回程", quality_label(internal), GREEN if internal["status"] == "PASS" else YELLOW)
    field("出口后端服务", f"{backend['status']}｜{backend['evidence']}", GREEN if backend["status"] == "PASS" else YELLOW)
    field("客户端出口闭环", f"{handshake['status']}｜{handshake['reason']}", GREEN if handshake["status"] == "PASS" else YELLOW)
    field("出口公网", f"{exit_match['status']}｜{mask_ip(identity.get('ip', ''))}", GREEN if exit_match["status"] == "PASS" else YELLOW)
    field(
        "三网单线程辅助",
        f"{speed['status']}｜{speed['reason']}｜不经过公网中转入口",
        CYAN if speed["status"] == "VALID" else YELLOW,
    )
    ...
    field("报告方式", "独立本地 HTML／JSON／Markdown（不复用其他脚本公共页）", CYAN)
    field("重要说明", "本工具仅用于公网入口＋隧道＋出口后端的中转链分层验证", MAGENTA)
    if SELF_TEST:
