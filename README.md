...
RC4.2.23 将完整测试结束后的自动上报与 `--retry-upload` 收敛为同一条浏览器式路径：直接读取已经落盘的原始 JSON，先取得报告站 Cookie，再以 curl HTTP POST 上传 `/api/reports`。不再把 RC 报告二次转换为另一份公共载荷；自动上报与浏览器选择 JSON 上传使用相同的数据。上传失败时本地 HTML／JSON 仍会保留。
...
```bash
bash <(curl -fsSL "https://raw.githubusercontent.com/souldance7-ai/vps-speedtest/main/3net-route.sh?ver=RC4.2.23") --extended --speed
```
...
> 当前版本：RC4.2.23 RAW-JSON-BROWSER-UPLOAD。公开使用前请自行确认业务端口及当地法律与服务商条款。
